        ...
        <div className="max-w-3xl mx-auto p-6 pb-24 md:pb-8">
            <div className="mb-8 animate-fade-up">
                <div className="flex items-center gap-3 mb-1">
                    <Heart className="w-6 h-6 fill-current" style={{ color: "var(--rose-accent)" }} />
                ...
                <p style={{ color: "var(--muted-foreground)" }}>
                    {favorites.length > 0
                        ? `${favorites.length} შენახული პროფილი`
            ...
            {favorites.length === 0 ? (
                ...
                >
                    ...
                    <p className="text-sm mt-1">პროფილებზე დააჭირეთ გულის ღილაკს შესანახად</p>
                </div>
            ) : (
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 animate-fade-up delay-100">
                    {favorites.map((fav) => {
                        const u = fav.favorite;
                        return (
                            <Link
                                key={u.id}
                                href={`/profile/${u.id}`}
                            ...
                            >
                                <div className="relative w-20 h-20 rounded-2xl overflow-hidden flex-shrink-0"
                                    ...
                                    {u.avatar ? (
                                        <Image src={u.avatar} alt={u.name ?? "User"} fill sizes="80px" className="object-cover transition-transform duration-500 group-hover:scale-110" />
                                    ) : (
